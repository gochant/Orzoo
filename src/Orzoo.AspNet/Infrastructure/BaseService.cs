        ...
        where T : class, IEntity<TKey>, new()
    {
        ...
        public virtual IQueryable<T> GetQuery(IQueryable<T> queryable = null, bool sorted = true, bool noTracking = true)
        {
            if (queryable == null)
            {
                ...
                if (sorted)
                {
                    if (typeof (IAutoGenerateDateEntity).IsAssignableFrom(typeof (T)))
                    {
                        queryable = queryable.Cast<IAutoGenerateDateEntity>().DateOrderStandard().Cast<T>(); // 默认按照创建时间降序排列
                    }
                    else
                    {
                        // 默认按照Id排序
