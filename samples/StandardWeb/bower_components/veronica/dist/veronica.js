...
define('app/widget',[
    ...
    return function (app) {
        ...
        function hasLocal(name) {
            ...
            || _.findIndex(app.existWidgets, function(val){
                return val.indexOf( name.replace(new RegExp('-', 'g'), '')) >= 0;
            }) < 0;
        }

        function getLocal(name) {
            var result = app.widget._localWidgetExes[name];
            if (!result) {
                _.each(app.widgetsPattern, function (value, key) {
                    _.each(value, function (regex) {
                        if (regex.test(name)) {
                            result = app.widget._localWidgetExes[key];
                        }
                    });
                });
            }
...
define('app/view/view-mvvm',[],function () {
    ...
    return function (app) {
        ...
        var methods = {
             ...
             */
            url: function (name) {
                var parts = this.options.sandbox.app.page.currName().split('/');
                var url = this.options.url[name];
                if (parts[1]) {
                    url = url.replace('{controller}', parts[1]);
                }
                return url;
            },
